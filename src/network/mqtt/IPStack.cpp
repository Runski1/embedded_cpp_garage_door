 ...
 * called!
 */
err_t IPStack::tcp_client_connected(void *arg, struct tcp_pcb *tpcb,
                                    err_t err) {
    auto state = static_cast<IPStack *>(arg);
    if (err != ERR_OK) {
...
err_t IPStack::tcp_client_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p,
    ...
    if (p->tot_len > 0) {
        ...
        uint16_t bytes_to_copy =
            available > p->tot_len ? p->tot_len : available;
        ...
        if (wr_end > BUF_SIZE) {
            ...
            first_copy = BUF_SIZE -
                         state->wr; // calculate the size of first part to copy
            if (first_copy) {       //
                bytes_to_copy -= pbuf_copy_partial(p, state->buffer + state->wr,
                                                   first_copy, 0);
...
int IPStack::read(unsigned char *buffer, int len, int timeout) {
    ...
    if (bytes_to_copy) {
        ...
        if (rd_end > BUF_SIZE) {
            // need to copy in two parts
            first_copy =
                BUF_SIZE - rd; // calculate the size of the first part to copy
            if (first_copy) {  //
